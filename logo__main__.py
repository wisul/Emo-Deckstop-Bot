...
async def start(bot, message):
   await message.reply_photo("hi"caption=START,reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Help", callback_data="help_menu"), InlineKeyboardButton(text="Repo", url="https://github.com/RishBropromax/Emo-Deckstop-Bot")]]))

@app.on_message(filters.command("help"))
async def help(bot, message):
   await message.reply_photo("hi", caption=HELP,reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Back", callback_data="start_menu")]]))

@app.on_message(filters.command("logo") & filters.incoming & filters.text & ~filters.forwarded & (
  filters.group | filters.private))
async def logo(bot, message):
  try:
    text = message.text.replace("logo","").replace("/","").replace("@Emo_Deckstop Bot","").strip().upper()
    ...
    await message.reply_photo(logo,caption="**🖼 Logo Generated By @Emo_Deckstop Bot",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Upload As File 📁", callback_data=f"flogo {logo_id}")]]))
    await x.delete()
  except FloodWait:
    pass
...
async def logo(bot, message):
  try:
    text = message.text.replace("logosq","").replace("/","").replace("@Emo_Deckstop_Bot","").strip().upper()
...
async def start_menu(_,query):
  await query.answer()
  await query.message.edit(START,reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Help", callback_data="help_menu"),InlineKeyboardButton(text="Repo", url="https://github.com/RishbroProMax/Emo-deckstop-Bot
...
async def logo_doc(_,query):
  ...
  try:
    ...
    await query.message.reply_document(link,caption="**🖼 Logo Generated By @Emo_Deckstop Bot**")
  except FloodWait:
    pass
  except Exception as e:
    try:
